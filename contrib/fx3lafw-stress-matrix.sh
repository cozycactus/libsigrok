...
CASES=${FX3_CASES:-"64M:32 80M:24 96M:16 192M:8"}
...
if [ -n "${FX3_DYLD_LIBRARY_PATH:-}" ]; then
	if [ -n "${DYLD_LIBRARY_PATH:-}" ]; then
		DYLD_LIBRARY_PATH=$FX3_DYLD_LIBRARY_PATH:$DYLD_LIBRARY_PATH
	else
		DYLD_LIBRARY_PATH=$FX3_DYLD_LIBRARY_PATH
	fi
	export DYLD_LIBRARY_PATH
fi

if [ -n "${FX3_LD_LIBRARY_PATH:-}" ]; then
	if [ -n "${LD_LIBRARY_PATH:-}" ]; then
		LD_LIBRARY_PATH=$FX3_LD_LIBRARY_PATH:$LD_LIBRARY_PATH
	else
		LD_LIBRARY_PATH=$FX3_LD_LIBRARY_PATH
	fi
	export LD_LIBRARY_PATH
fi

if [ -n "${FX3_FIRMWARE_DIR:-}" ]; then
	SIGROK_FIRMWARE_DIR=$FX3_FIRMWARE_DIR
	export SIGROK_FIRMWARE_DIR
fi
