...
#ifndef LIBSIGROK_HARDWARE_FX3KIT_PROTOCOL_H
#define LIBSIGROK_HARDWARE_FX3KIT_PROTOCOL_H
...
struct dev_context {
	const struct fx3kit_profile *profile;
...
SR_PRIV int fx3kit_dev_open(struct sr_dev_inst *sdi, struct sr_dev_driver *di);
SR_PRIV struct dev_context *fx3kit_dev_new(void);
SR_PRIV int fx3kit_start_acquisition(const struct sr_dev_inst *sdi);
SR_PRIV void fx3kit_abort_acquisition(struct dev_context *devc);

SR_PRIV int fx3kit_receive_data(int fd, int revents, void *cb_data);
